...
- This is not new, but it's worth pointing out: you can save the contents of the Tape Viewer window by clicking the 'Save' pushbutton at the top left of the window. There won't be any acknowledgment of the button press, but you'll get a file in the ..\Output directory with the extension '.DMP', named according to the tape image file being displayed (e.g., 'SYSOUT.BCD.DMP' or 'SYSIN.BCD.DMP').  The middle part of the dump file name is always '...BCD...' but in fact the image will be dumped in whatever mode the Tape Viewer is currently displaying; either BCD or BIN.
