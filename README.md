...
Note that this "Include File" command is surrounded by text constituting the IBSYS control cards that precede and follow the actual user-program source code.

If no path is given, the scripter "Include File" command will look for the file in the ..\Files\Cards directory. This is true even if, as is the case here, the program and its enclosing control cards have to be converted to a tape before the job can run, since the FORTRAN II subsystem does not permit job input from cards (this conversion happens slightly further on in the "Xample_Sysdmp.Fap.KSYS.EC7" script file you currently have open in the Editor).  There are two alternatives to having your file in the expected directory: a) use the full path name in the "Include File" command or b) add to or change the list of default search paths on the B7094 "Configuration" window (click the "Config" checkbox on the Control Panel to bring it up). You might in fact want to create ..\Files\MyScripts and ..\Files\MyCards directories underneath your B7094 installation directory, and add these directories to the list of search paths on the Configuration window.
