...
For the paranoid, there's also a file "SHA-512_checksums.txt" containing checksums generated by a Windows utility:
...
The emulator is fairly robust. If you do something silly, like clicking 'Drop All Drives' while the CPU is operating, you'll get a warning message "CPU/Channel Check exists. Continue running script?". Just click 'Yes' at that point (and answer either 'No' or 'Yes' to the subsequent "Save current trace data?" question) and the demo script will reset the CPU automatically and re-display the menu from which you started the program you so rudely interrupted. If you answer 'No', the demo script (or whatever script you're running) will terminate (and the 'Stop Script' button in the top row of controls in the Control Panel will disappear). You could then click 'Clear' on the Console to clear the error and reset the CPU, answer the "Save current trace data?" question, bring up the Editor, 'Open' and 'Run' ..\Files\Scripts\B7Demo.EC7 (or any other script file), and you'd be back in business.
